...
| Household listings and selected pickup | Implemented / partial | `supplyChainRoutes.ts` listing, pickup, cancellation, reassignment options/commit and settlement payment; `HouseholdSupplyScreen.kt` | Disputed-inventory reconciliation, route-level proof, distance/reliability presentation and data-safe evidence capture remain partial. |
...
| Recycler demand | Implemented / partial | `ProcurementRequirement`, recycler create/list routes, demand intelligence and pooling suggestions | Live integration, notification fan-out policy and geospatial indexing remain incomplete. |
| Recycler bulk lots/offers/receive | Implemented / partial | `BulkLot`, `BulkOffer`, formal QR handover/QC/payment routes and `RecyclerSupplyScreen.kt` | Direct receive bypass is blocked. QC/settlement evidence and formal payment/earnings linkage exist; provider execution and review UI remain incomplete. |
...
| Offline dual-confirmation handover | Implemented / partial | `SupplyHandover`, signed QR, role-aware `/sync`, formal cache and Android handover screens | Backend formal pool/settlement/payment/evidence operations are queueable; shared discovery, Android wiring and fully local QR cryptographic verification remain online-bound. |
| Material Passport | Implemented / partial | `MaterialPassportEvent`, `AuditEvent`, `MATERIAL_PASSPORT.md`, formal handover/payment/QC endpoints | Live formal chain has event/hash/reference/quantity/payment evidence. Official certification, external notarisation and provider settlement are not claimed. |
...
| Data-safe household disposal | Implemented / partial | Hazardous-material acknowledgement, preparation states, passport evidence and Recycler disposal-evidence endpoint | Physical evidence authenticity/storage and Android/operator capture remain incomplete. Never claim destruction without Recycler evidence. |
...
| Sync operation integrity | Implemented / partial | `SyncOperation`, `/sync`, `SyncService`, `SyncQueue`, formal handover queue | Legacy and formal pool/settlement/handover/disposal/payment operations are supported with role/validation/replay guards; shared discovery remains online-only. |
