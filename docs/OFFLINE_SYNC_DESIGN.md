...
| Recycler supply receipt | Signed QR text plus a queued `CONFIRM_SUPPLY_HANDOVER` operation | Worker retries the exact handover confirmation; the server's conditional state transition makes replay harmless | A disconnected recycler cannot receive a final server settlement immediately |
...
The new supply-handover queue is restricted to a Recycler confirmation. A
Collector cannot enqueue a fake receipt, and a Recycler cannot use the queue to
invoke collector-only operations. Retryable network/server failures remain
pending; validation, forbidden and terminal conflict errors are surfaced for
operator attention.
...
those operations are intentionally online-only. QR display can be cached, but
the prototype does not perform the HMAC verification entirely on-device before
reconnect. Background sync behavior is tested on the emulator, not yet on a
