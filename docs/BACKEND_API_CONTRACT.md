...
- `Idempotency-Key` is accepted on pickup creation and formal handover confirmations. It must be 8–160 characters matching `[A-Za-z0-9._:-]+`. Reusing a key with a different request body returns `409 IDEMPOTENCY_KEY_REUSED`; replaying the same operation returns the canonical prior response.
...
- Formal supply handovers are QR/nonce protected and use `GENERATED`, `CONFIRMED_BY_RECYCLER`, `DISPUTED`, `COMPLETED`, `PENDING_MANUAL_REVIEW`, and `EXPIRED` states. Material/weight/rate changes require a reason and may require manual review.
...
| `POST /sync` | Collector | Queued operation batch with client IDs/timestamps. | Per-operation applied/rejected/conflict/invalid result. | Offline queue replay; server validation and ownership still apply. |
| `GET /sync/changes` | Collector | Optional cursor/timestamp. | Collector-scoped deltas including pickups, inventory movements, lots, pools and contributions. | Cursor is read-only; no cross-collector rows. |
...
| `GET /household/kabadiwalas` | Household | None. | Active Kabadiwala public-safe profiles. | No private contact/evidence fields. |
...
| `GET /kabadiwala/route-advantage` | Collector | Material/location/quantity query. | Route economics: baseline range, confidence, demand relevance, minimum lot, payment-reliability proxy, platform fee, gross/net estimates, reasons and `whyThisMatch`. | Deterministic read-only estimate; does not promise a price or payout. |
