...
- `ANDROID IMPLEMENTATION REQUIRED` means the contract is exposed or documented, but the current app has no safe role surface yet. No current admin capability is in this category; remaining `PARTIAL` entries are detail/test gaps.
...
| `POST /auth/admin-login` | Public | Onboarding operator sign-in → AdminConsoleScreen | T | L/C/R; role-safe operator route | COMPLETE |
...
| `GET /admin/recyclers` | Admin + capability | AdminConsoleScreen → AdminConsoleViewModel | J | Pending queue with review action | COMPLETE |
| `GET /admin/recyclers/:recyclerId` | Admin + capability | Local record details in AdminConsoleScreen | J | Remote detail endpoint remains optional for deeper inspection | PARTIAL |
| `PUT /admin/recyclers/:recyclerId/authorization` | Admin + capability | Authorization dialog with evidence/freshness fields | J | Explicit status/evidence form | COMPLETE |
...
| `GET /admin/disputes` | Admin + capability | AdminConsoleScreen → AdminConsoleViewModel | J | Open queue and resolve dialog | COMPLETE |
| `GET /admin/disputes/:disputeId` | Admin + capability | Queue record details | J | Remote detail endpoint remains optional for deeper inspection | PARTIAL |
| `POST /admin/disputes/:disputeId/resolve` | Admin + capability | Resolution dialog with notes | J | Server capability/confirmation | COMPLETE |
...
| `GET /admin/payments` | Admin + capability | AdminConsoleScreen → AdminConsoleViewModel | J | Pending queue and action dialog | COMPLETE |
| `GET /admin/payments/:paymentId` | Admin + capability | Queue record details | J | Remote detail endpoint remains optional for deeper inspection | PARTIAL |
| `POST /admin/payments/:paymentId/verify` | Admin + capability | Verify action with operator confirmation | J | Server capability/confirmation | COMPLETE |
...
| `POST /kabadiwala/pickups/:pickupId/reject` | Collector | PickupCard decline → reason dialog | T | L/C/R | COMPLETE |
| `POST /kabadiwala/pickups/:pickupId/confirm-availability` | Collector | PickupCard ready action | T | L/C/R | COMPLETE |
| `POST /kabadiwala/pickups/:pickupId/schedule` | Collector | PickupCard schedule dialog | T | L/C/R | COMPLETE |
| `POST /kabadiwala/pickups/:pickupId/cancel` | Collector | PickupCard cancel → reason dialog | J | L/C/R | COMPLETE |
| `POST /kabadiwala/pickups/:pickupId/reassign` | Collector | PickupCard no-show → reassignment dialog | T | Reason/no-show fields | COMPLETE |
...
| `POST /kabadiwala/bulk-offers/:offerId/reject` | Collector | OfferCard reject → reason dialog | T | L/C/R | COMPLETE |
| `POST /kabadiwala/bulk-offers/:offerId/counter` | Collector | OfferCard counter → rate/notes dialog | T | L/C/R | COMPLETE |
...
| `POST /recycler/offers/:offerId/withdraw` | Verified Recycler | RecyclerOfferCard withdraw action | T | L/C/R | COMPLETE |
...
| `PATCH /recycler/procurement-requirements/:requirementId` | Verified Recycler | RequirementEditDialog | T | L/C/R | COMPLETE |
...
| `GET /kabadiwala/pools/suggestions` | Collector | FormalisationDashboard suggestion cards | T | L/C/E/R | COMPLETE |
...
| `GET /kabadiwala/demand-intelligence` | Collector | FormalisationDashboard demand intelligence card | J | L/C/E/R | COMPLETE |
| `GET /kabadiwala/passport` | Collector | Growth Passport card | T | L/C/E/R/O | COMPLETE |
| `GET /safety-routing` | Collector | FormalisationDashboard material/condition route card | T | L/C/R | COMPLETE |
...
| `POST /kabadiwala/handovers/:handoverId/settlement` | Collector | Handover card settlement review dialog | T | Reason/evidence/notes fields | COMPLETE |
| `GET /kabadiwala/handovers/:handoverId/passport` | Collector | Handover card passport action | T | Inline evidence; dedicated destination remains partial | PARTIAL |
| `GET /kabadiwala/handovers/:handoverId/anomalies` | Collector | Handover card risk flags action | T/J | Inline anomaly summary; dedicated explanation remains partial | PARTIAL |
| `GET /admin/formal-anomalies` | Admin + `DISPUTE_RESOLUTION` | AdminConsoleScreen anomaly queue | J | Open queue and audited resolution dialog | COMPLETE |
| `POST /admin/formal-anomalies/:flagId/resolve` | Admin + `DISPUTE_RESOLUTION` | AdminConsoleViewModel resolution action | J | Action, resolution and evidence fields | COMPLETE |
| `POST /admin/formal-payments/:paymentId/reverse` | Admin + `PAYMENT_VERIFICATION` | AdminConsoleScreen payment reversal action | J | Reason/provider/reference/evidence fields | COMPLETE |
...
| Endpoint | Role | Existing screen/repository/ViewModel | Model | Status |
|---|---|---|---|---|
| `POST /admin/datasets/prices/import` | Admin + `DATASET_EXPORT` | AdminTools → validated price-row dialog | J | Server validation and audit | COMPLETE |
| `GET /admin/datasets/export` | Admin + `DATASET_EXPORT` | AdminTools export action | J | Server pseudonymization; local download UX remains partial | PARTIAL |
