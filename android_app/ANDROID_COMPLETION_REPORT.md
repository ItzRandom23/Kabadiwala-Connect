...
The Android application now builds on the existing Compose/MVVM foundation with a dark-first Neon Purple theme, the household/pickup/inventory/formal-handover contract surface, persisted idempotency keys, offline pickup/handover replay, and a clear compatibility guard against using the legacy recycler receipt endpoint. The operator/admin role is also now authenticated through `/auth/admin-login` and routed to a capability-gated console.
...
- `ui/screens/supplychain/SupplyChainScreens.kt`: household data-bearing device preparation/destruction-evidence request fields plus visible pickup reliability, offer lifecycle, demand, safety, passport, anomaly and settlement actions.
- `ui/screens/admin/AdminConsoleViewModel.kt`, `ui/screens/admin/AdminConsoleScreen.kt`: admin-only recycler authorization, dispute resolution, payment verification/reversal, formal-anomaly resolution, validated price import/update and pseudonymized dataset export.
- `ui/navigation/AppNavHost.kt`, `ui/navigation/Destinations.kt`, `data/auth/*`: role-safe admin entry, persistence and navigation guard.
...
- New pickup reliability, reschedule, household settlement, inventory movement, offer lifecycle, demand update, safety-routing, passport and anomaly paths are typed and wired into visible role screens. Dedicated deep-detail destinations remain partial.
- Admin login and the capability-gated operator console now expose recycler review, disputes, payments, formal anomalies and data tools. The server remains authoritative for each permission and role check.
...
| Check | Result |
|---|---|
| Debug unit tests | PASS — `testDebugUnitTest`, 72 tests across 12 suites |
| Android lint | PASS — `lint`; 1 new API-level error fixed, existing warnings remain |
...
1. Household listing → Kabadiwala → pickup → settlement → passport: `IMPLEMENTED WITH DETAIL LIMITATION`; primary listing/pickup/reschedule/settlement actions are visible; a dedicated source-passport detail route remains.
2. Kabadiwala pickup → inventory → pool → formal handover: `IMPLEMENTED`; lifecycle controls, route/demand/safety guidance, pool actions and QR handover are visible.
3. Recycler demand → offer → QR scan → QC/settlement: `IMPLEMENTED`; demand editing, offer withdrawal, QR review and formal settlement/QC payloads are connected; a dedicated historical detail view remains.
...
5. Settlement differences and anomaly review: `IMPLEMENTED`; collector review cards and admin anomaly/dispute/payment queues expose reason/evidence actions.
6. Wrong role/non-owned data: `IMPLEMENTED` at navigation guard/cache scoping plus backend ownership enforcement; Admin has a separate role-safe route.
...
- `ANDROID PARTIAL`: dedicated detail destinations for household listing/pickup passports, pickup history, handover passport and anomaly explanation.
- `ANDROID PARTIAL`: some secondary recycler/collector history views and capability-denied explanatory screens.
