...
export const handoverController = (s: HandoverService, db?: PrismaClient) => ({
  ...
  create: async (q: Request, r: Response) => { const p = z.object({ lotId: z.string(), quoteId: z.string(), clientHandoverId: z.string().trim().min(16).max(120).optional(), handoverLocation: location, timestamp: z.string().optional() }).safeParse(q.body); if (!p.success) throw new AppError('VALIDATION_ERROR', 'Invalid handover', 422, { code: 'INVALID_HANDOVER_LOCATION' }); const data = await s.create(q.identity!.collectorId, p.data); if (db) await emitNotification(db, { accountId: q.identity!.collectorId, type: 'HANDOVER_CREATED', title: 'Handover ready', body: 'Your signed handover record is ready for verification.', route: `handovers/document/${data.id}`, dedupeKey: `HANDOVER_CREATED:${data.id}` }); return r.status(201).json({ success: true, data, message: 'Handover generated' }); },
  ...
  dispute: async (q: Request, r: Response) => {
    ...
    if (db) await emitNotification(db, { accountId: q.identity!.collectorId, type: 'HANDOVER_DISPUTED', title: 'Dispute opened', body: 'Your handover evidence is queued for review.', route: `disputes/analytics`, dedupeKey: `HANDOVER_DISPUTE:${data.id}` });
