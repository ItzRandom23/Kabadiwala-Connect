...
const createSchema = z.object({ materialCategory: z.enum(['CRT', 'LCD_PANEL', 'PCB', 'CABLE', 'BATTERY', 'MOTOR', 'MAGNET', 'PLASTIC', 'OTHER']), materialSubcategory: z.string().trim().max(160).optional(), condition: z.enum(['INTACT', 'DAMAGED', 'PARTIAL']), weight: z.number().finite().positive().lt(500), collectionLocation: location, notes: z.string().max(1000).optional() });
