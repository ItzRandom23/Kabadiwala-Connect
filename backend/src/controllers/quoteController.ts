...
export const quoteController = (s: QuoteService, db?: PrismaClient) => ({
  request: async (q: Request, r: Response) => {
    ...
    if (db) await emitNotification(db, { accountId: p.data.recyclerId, type: 'QUOTE_REQUESTED', title: 'New quote request', body: 'A collector requested a quote for a verified material lot.', route: 'recycler/marketplace' });
  ...
  requestBatch: async (q: Request, r: Response) => {
    ...
    if (db) await Promise.all(data.requested.map((request: any) => emitNotification(db, { accountId: request.recyclerId, type: 'QUOTE_REQUESTED', title: 'New quote request', body: 'A collector requested a quote for a verified material lot.', route: 'recycler/marketplace' })));
  ...
  reject: async (q: Request, r: Response) => {
    ...
    if (db) await emitNotification(db, { accountId: data.recyclerId, type: 'QUOTE_REJECTED', title: 'Quote declined', body: 'The collector declined your quote. Review the marketplace for another request.', route: 'recycler/marketplace' });
  ...
  submit: async (q: Request, r: Response) => {
    ...
    if (db && request) await emitNotification(db, { accountId: request.collectorId, type: 'QUOTE_RECEIVED', title: 'New quote received', body: 'A verified recycler sent a quote for your lot.', route: `quotes/compare/${data.lotId}` });
