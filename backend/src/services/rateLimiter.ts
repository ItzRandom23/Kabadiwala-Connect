...
export class OtpRateLimiter implements AuthenticationRateLimiter { private readonly identifiers = new Map<string, Entry>(); private readonly ips = new Map<string, Entry>(); async check(identifier: string, ip: string, kind: 'request'|'verify'|'login'|'signup'|'public_verify') { const now = Date.now(); const check = (map: Map<string, Entry>, key: string, max: number, window: number, cooldown = 0) => { const old = map.get(key); const item = !old || old.resetAt <= now ? { count: 0, resetAt: now + window } : old; if (cooldown && item.lastAt && now - item.lastAt < cooldown) throw rateLimitError('OTP_COOLDOWN', 'Please wait before trying again', item.lastAt + cooldown, now); if (item.count >= max) throw rateLimitError('OTP_RATE_LIMITED', 'Too many requests. Try again later.', item.resetAt, now); item.count++; item.lastAt = now; map.set(key, item); }; if (kind === 'request') { check(this.identifiers, identifier, 5, 60 * 60_000, 30_000); check(this.ips, ip, 20, 60 * 60_000); } else if (kind === 'public_verify') check(this.ips, ip, 30, 15 * 60_000); else { check(this.identifiers, identifier, 10, 15 * 60_000); check(this.ips, ip, 30, 15 * 60_000); } } }
  ...
  async check(identifier: string, ip: string, kind: 'request'|'verify'|'login'|'signup'|'public_verify') {
    ...
    for (const [scope, value, limit] of values) {
      ...
      if (kind === 'request' && scope === 'identifier' && existing?.lastAt) {
        const cooldownUntil = existing.lastAt.getTime() + 30_000;
