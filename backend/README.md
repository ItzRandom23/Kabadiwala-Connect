...
Gemini features are optional in testing and fail safely when the key is absent or the provider is unavailable. Set `GEMINI_API_KEY` and optionally `GEMINI_MODEL` (the supported default is `gemini-3.5-flash-lite`, which accepts image input and structured output through Google's `generateContent` API). The key remains server-side. Lot descriptions still fall back to a deterministic template; photo material identification falls back to `OTHER` and always requires collector confirmation. Transaction chat remains human-to-human: `POST /api/v1/future/conversations/:conversationId/draft-reply` returns an editable Gemini reply draft and never sends or stores it automatically. Before enabling production, verify the configured key and model with a staging material-suggestion request; an invalid/expired key or unsupported model is surfaced to the app as a manual-selection fallback.
